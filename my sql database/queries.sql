...
ALTER TABLE users
ADD COLUMN role ENUM('user','buddy','admin') NOT NULL DEFAULT 'user' AFTER password;
...
ADD COLUMN profession VARCHAR(100) NULL AFTER address,
ADD COLUMN vehicle_type VARCHAR(50) NULL AFTER profession,
ADD COLUMN vehicle_number VARCHAR(50) NULL AFTER vehicle_type,
ADD COLUMN is_verified BOOLEAN DEFAULT FALSE AFTER vehicle_number;
...
CREATE TABLE rides (
    ...
    ride_type ENUM('walking','outing','hangout') NOT NULL,
    scheduled_at DATETIME NULL,
    status ENUM('pending','accepted','ignored','completed','cancelled') DEFAULT 'pending',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
